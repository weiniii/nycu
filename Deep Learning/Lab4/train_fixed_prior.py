...
from utils import init_weights, kl_criterion, finn_eval_seq, pred
...
#     parser.add_argument('--model_dir', default='./logs/lp/rnn_size=256-predictor-posterior-rnn_layers=2-1-n_past=2-n_future=10-lr=0.0020-g_dim=128-z_dim=64-last_frame_skip=False-beta=0.0001000', help='base directory to save logs')
...
#     for epoch in range(start_epoch, start_epoch + 49):
...
#             epoch_kld += kld
        ...
        kl_weight_list.append(kl_anneal.get_beta())
        ...
        if epoch % 5 == 0 or epoch == start_epoch+niter-1:
            ...
            print(f'avergae psnr: {ave_psnr}')
            avgpsnr_list.append(ave_psnr)
        ...
        torch.save(avgpsnr_list,'%s/avgpsnr.pt' % args.log_dir)
        torch.save(loss_list,'%s/loss.pt' % args.log_dir)
        torch.save(kl_weight_list,'%s/kl_weight.pt' % args.log_dir)
        torch.save(tfr_list,'.%s/tfr.pt' % args.log_dir)
