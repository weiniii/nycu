...
def value_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    # 3. Iterate and improve V(s) using the Bellman optimality operator
    for t in tqdm.tqdm(range(max_iterations)):
        old_V = np.array(V)
        for s in range(num_spaces):
            V_list = [ gamma * P[s, a] @ V + sum(R[s, a]) for a in range(num_actions)]
            V[s] = np.max(V_list)
        if np.linalg.norm((V - old_V), np.inf) < eps:
            break
    # 4. Derive the optimal policy using V(s)
    for s in range(num_spaces):
        update_policy = np.array([ gamma * P[s, a] @ V + sum(R[s, a]) for a in range(num_actions)])
        policy[s] = np.argmax(update_policy)
    #############################
...
def policy_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    for t in tqdm.tqdm(range(max_iterations)):
        ...
        for s in range(num_spaces):
            Q_list = np.array([ gamma * P[s, a] @ V + sum(R[s, a]) for a in range(num_actions)])
            policy[s] = np.argmax(Q_list)
        for t2 in range(max_iterations):
            old_V = np.array(V)
            for s in range(num_spaces):
                a = policy[s]
                V[s] = sum(R[s, a]) + gamma * P[s, a] @ V
            if np.linalg.norm((V - old_V), np.inf) < eps:
                break
