...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.999, lambda_=0.99, n_step=None):
        """
        ...
        init = GAE(gamma, lambda_, n_step)
        returns = init(self.rewards, saved_actions, True)
        
        returns = (returns - returns.mean()) / (returns.std() + eps)

        for log_prob, R in zip(saved_actions, returns):
            policy_losses.append(-log_prob[0] * R)
            value_losses.append(R)
...
class GAE:
    ...
    def __call__(self, rewards, values, done):
        ...
        for reward, value in zip(reversed(rewards), reversed(values)):
            td_errer = reward + next_value * self.gamma - value[1]
            advantage = td_errer + advantage * self.gamma * self.lambda_
            try:
                advantage -= advantages[self.num_steps] * self.gamma ** self.num_steps
            except:
                pass
            next_value = value[1]
            advantages.insert(0, advantage)

        advantages = torch.tensor(advantages) * self.lambda_
